...
//
G4_DECLARE_PHYSCONSTR_FACTORY(EmStandardPhysics_option4);
...
//================================================================================================================================
  ...
  // G4bool polar = param->DisablePolarisation();

  // Photoelectric
  G4PhotoElectricEffect* ppe = new G4PhotoElectricEffect();
  G4VEmModel* ppeModel = new G4LivermorePhotoElectricModel();
  ppe->SetEmModel(ppeModel);
  if(polar) {
    ppeModel->SetAngularDistribution(new G4PhotoElectricAngularGeneratorPolarized());
  }

  // Compton scattering
  G4ComptonScattering* pcs = new G4ComptonScattering;
  pcs->SetEmModel(new G4KleinNishinaModel());
  G4VEmModel* pcModel = nullptr;
  if(polar) { 
    pcModel = new G4LowEPPolarizedComptonModel();
  } else {
    pcModel = new G4LowEPComptonModel();
  }
  pcModel->SetHighEnergyLimit(20*CLHEP::MeV);
  pcs->AddEmModel(0, pcModel);

  // Gamma conversion
  G4GammaConversion* pgc = new G4GammaConversion();
  G4VEmModel* pconv = new G4BetheHeitler5DModel();
  pgc->SetEmModel(pconv);

  // default Rayleigh scattering is Livermore
  G4RayleighScattering* prl = new G4RayleighScattering();
  if(polar) {
    prl->SetEmModel(new G4LivermorePolarizedRayleighModel());
  }

  if(param->GeneralProcessActive()) {
    G4GammaGeneralProcess* psp = new G4GammaGeneralProcess();
    psp->AddEmProcess(ppe);
    psp->AddEmProcess(pcs);
    psp->AddEmProcess(pgc);
    psp->AddEmProcess(prl);
    G4LossTableManager::Instance()->SetGammaGeneralProcess(psp);
    ph->RegisterProcess(psp, particle);
  } else {
    ph->RegisterProcess(ppe, particle);
    ph->RegisterProcess(pcs, particle);
    ph->RegisterProcess(pgc, particle);
    ph->RegisterProcess(prl, particle);
  }
